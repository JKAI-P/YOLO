...
def check_all_completed():
    """检查所有实验是否完成."""
    base_dir = "runs/train"
    experiments = ["1_baseline", "2_cbam", "3_cbam_wf", "4_iyuav_det", "comp_yolov5s", "comp_yolov7"]
        ...
        if check_all_completed():
            ...
            print(
                f"\n📊 进度: {
                    sum(
                        1
                        for exp in ['1_baseline', '2_cbam', '3_cbam_wf', '4_iyuav_det', 'comp_yolov5s', 'comp_yolov7']
                        if os.path.exists(f'runs/train/{exp}/results.csv')
                        and sum(1 for _ in open(f'runs/train/{exp}/results.csv')) >= 301
                    )
                }/6 个实验完成"
            )
