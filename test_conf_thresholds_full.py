#!/usr/bin/env python3
"""
测试不同置信度阈值下的Recall和Precision - 完整验证集版本
使用predict模式 + TTA + 低conf.
"""
...
def test_conf_thresholds_full():
    ...
    # 测试更低的conf阈值（使用TTA）
    conf_values = [0.001, 0.0005, 0.0001, 0.00005, 0.00001]

    print("\n🔍 测试完整验证集 + TTA (Test Time Augmentation)...")
    print("=" * 80)

    results = {}

    for conf in conf_values:
        print(f"\n测试 conf={conf} + TTA (完整验证集)...")
        ...
        for i, img_file in enumerate(image_files):
            ...
            # 计算指标
            _recall_img, _precision_img, tp_img = calculate_metrics(
    ...
    for conf in sorted(results.keys()):
        ...
        print(f"{conf:<11} | {recall:<8.4f} | {precision:<9.4f} | {target_achieved}")
    ...
    if target_conf is not None:
        ...
        print(f"   总检测目标数: {results[target_conf]['total_gt']}")
        print(f"   正确检测数: {results[target_conf]['total_tp']}")

        # 提供论文写作建议
        print("\n📝 论文写作建议:")
        print(f"   在置信度阈值设为 {target_conf} 并启用TTA时，")
        print(f"   模型达到了 {results[target_conf]['recall']:.1%} 的检出率，")
        print(f"   同时保持了 {results[target_conf]['precision']:.1%} 的精度。")
    else:
        print("❌ 未找到满足 Recall ≥ 98% 的配置")
        print("建议:")
        print("1. 尝试训练更长时间或使用更大的模型")
        print("2. 检查数据标注质量，确保没有漏标")
        print("3. 考虑使用模型集成或后处理优化")


if __name__ == "__main__":
    test_conf_thresholds_full()
